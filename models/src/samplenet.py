...
from .attention import NONLocalBlock1D, NONLocalBlock1D_mutual
...
class SampleNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.backbone == 'pointnet':
            ...
            y = F.relu(self.bn5(self.conv5(y)))  # Batch x 256 x NumInPoints
            y = self.mutual_attention(y)
    ...
    def get_repulsion_loss(self, ref_pc, samp_pc, threshold=5):
        loss = self.project.repulsion_loss(ref_pc, samp_pc, threshold=threshold)
        return loss
